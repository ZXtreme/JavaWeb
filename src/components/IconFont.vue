<template>
  <div class="iconFont">
    <svg class="icon" aria-hidden="true" v-for="(item, index) in icons" :key="index" :id="index" @click="pushIcon">
...
export default {
  ...
  methods: {
    ...mapMutations(['pushReadySendText']),
    pushIcon(e) {
      this.tabEvent()
      this.pushReadySendText(`[${e.currentTarget.id}]`)
    }
  }
}
</script>
